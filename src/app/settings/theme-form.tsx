...
const OPTIONS: { value: Theme; label: string; description: string }[] = [
  ...
  { value: "dev", label: "Dev", description: "Esthétique terminal, police mono." },
];
...
export function ThemeForm({ current }: { current: Theme }) {
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-lg border border-neutral-200 p-6 dark:border-neutral-800">
      <h3 className="font-medium text-neutral-900 dark:text-neutral-100">Thème</h3>
      ...
      <div className="space-y-2">
        {OPTIONS.map((option) => (
          ...
          >
            ...
            />
            <span>
              <span className="block font-medium text-neutral-900 dark:text-neutral-100">{option.label}</span>
