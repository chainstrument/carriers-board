  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-sm">
        <label htmlFor={criterionKey} className="text-neutral-600 dark:text-neutral-400">
          {label}
        </label>
        <span className="font-medium text-neutral-900 dark:text-neutral-100">{value}/10</span>
      </div>
      <input
        id={criterionKey}
        name={criterionKey}
...
export function EntryForm({
  ...
  const defaultMonth = defaultValues ? defaultValues.month.slice(0, 7) : currentMonthIso();

  return (
    <form action={formAction} className="space-y-6">
      <div className="space-y-1">
        <label htmlFor="month" className="text-sm text-neutral-600 dark:text-neutral-400">
          Mois
        </label>
        <input
          id="month"
