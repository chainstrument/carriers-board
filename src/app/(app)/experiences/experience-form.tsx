...
export function ExperienceForm({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const isCurrent = (form.elements.namedItem("isCurrent") as HTMLInputElement)
      .checked;
    ...
    <form action={formAction} onSubmit={handleSubmit} className="space-y-6">
      ...
      <label className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
        <input
          type="checkbox"
          name="isCurrent"
          defaultChecked={!defaultValues?.endDate && !!defaultValues}
        />
        Poste actuel
      </label>
      ...
      <Field id="missions" label="Missions">
        <textarea
          ...
          name="missions"
          rows={4}
          defaultValue={defaultValues?.missions ?? ""}
          className={inputClass}
        />
      </Field>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field id="positives" label="Points positifs">
          <textarea
            ...
            name="positives"
            rows={3}
            defaultValue={defaultValues?.positives ?? ""}
            className={inputClass}
          />
        </Field>
        <Field id="negatives" label="Points négatifs">
          <textarea
            ...
            name="negatives"
            rows={3}
            defaultValue={defaultValues?.negatives ?? ""}
            className={inputClass}
          />
        </Field>
      </div>
