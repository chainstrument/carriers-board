      ...
      {entries.length >= 2 && (
        <section className="mb-12 space-y-8">
          <div>
            ...
            <Sparkline
              values={entries.map((e) => averageScore(e))}
              width={480}
              height={80}
            />
            <div className="mt-1 flex justify-between text-xs text-neutral-400">
              <span>
                {shortMonthFormatter.format(new Date(entries[0].month))}
              </span>
              <span>
                {shortMonthFormatter.format(
                  new Date(entries[entries.length - 1].month),
                )}
              </span>
            </div>
          </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {CRITERIA.map((c) => (
                ...
                >
                  ...
                  <Sparkline
                    values={entries.map((e) => e[c.key] as number)}
                    width={220}
                    height={36}
                  />
                </div>
